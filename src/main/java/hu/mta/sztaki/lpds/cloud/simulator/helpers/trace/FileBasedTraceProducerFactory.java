...
public class FileBasedTraceProducerFactory {
	 ...
	 * extensions.
	 * 
	 * @param fileName    the name of the file to be parsed and loaded for jobs
	 * @param from        the first job to be included from the trace
	 * @param to          the last job to be included from the trace
	 * @param furtherjobs <i>true</i> if it is allowed to look further in the trace
	 * @param maxProcs    Only used by producers which are capable to scale their
	 *                    jobs in processor count
	 * @param jobType     the kind of jobs to instantiate
	 * @return The trace producer. <b>Warning:</b> If the trace producer kind could
	 *         not be determined then the function returns with <i>null</i>.
	 * @throws SecurityException        if the jobType cannot be instantiated
	 *                                  correctly
	 * @throws NoSuchMethodException    if the jobType cannot be instantiated
	 *                                  correctly
	 * @throws IOException              file reading problem
	 * @throws TraceManagementException if a random trace generation behaves
	 *                                  unexpectedly
	 */
